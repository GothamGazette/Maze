...
function kh_acl_check($aro, $aco, $axo = null)
{
    ...
    if (!is_array($tables))
    {
        ...
        $tables = array(
            ...
            'axos'           => 'acl_axos',
            'access'         => 'acl_access',
            'access_actions' => 'acl_access_actions'
        );
    ...
    $rs = $ci->db->query('SELECT allow FROM '.$ci->db->dbprefix.$tables['aros'].' AS tree
        JOIN '.$ci->db->dbprefix.$tables['aros'].' AS branch ON ( tree.name = ? AND branch.lft <= tree.lft AND branch.rgt >= tree.rgt )
        JOIN acl_access AS access ON ( access.aro_id = branch.id )
        JOIN '.$ci->db->dbprefix.$tables['acos'].' AS acostree ON ( acostree.name = ? )
        JOIN '.$ci->db->dbprefix.$tables['acos'].' AS acosbranch ON ( acosbranch.lft <= acostree.lft AND acosbranch.rgt >= acostree.rgt )
        WHERE access.aco_id = acosbranch.id
        ORDER BY branch.rgt, acosbranch.rgt
        LIMIT 0,1', array($aro, $aco));
    ...
    if ($rs->num_rows() == 1)
    {
        ...
        if ($row->allow == 'Y')
        {
            if ($axo !== null)
            {
                ...
                $rs = $ci->db->query('SELECT access_actions.allow FROM '.$ci->db->dbprefix.$tables['aros'].' AS tree
                    JOIN '.$ci->db->dbprefix.$tables['aros'].' AS branch ON ( tree.name = ? AND branch.lft <= tree.lft AND branch.rgt >= tree.rgt )
                    JOIN acl_access AS access ON ( access.aro_id = branch.id )
                    JOIN '.$ci->db->dbprefix.$tables['acos'].' AS acostree ON ( acostree.name = ? )
                    JOIN '.$ci->db->dbprefix.$tables['acos'].' AS acosbranch ON ( acosbranch.lft <= acostree.lft AND acosbranch.rgt >= acostree.rgt )
                    JOIN '.$ci->db->dbprefix.$tables['access_actions'].' AS access_actions ON ( access.id = access_actions.access_id ) 
                    JOIN '.$ci->db->dbprefix.$tables['axos'].' AS axos ON ( axos.id = access_actions.axo_id ) 
                    WHERE access.aco_id = acosbranch.id
                    ORDER BY branch.rgt, acosbranch.rgt
                    LIMIT 0,1', array($aro, $aco, $axo));
