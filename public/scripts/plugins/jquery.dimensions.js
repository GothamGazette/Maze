 ...
 * 
 * $LastChangedDate: 2007-06-22 04:38:37 +0200 (Fr, 22 Jun 2007) $ $Rev: 2141 $
 * 
 * Version: 1.0b2
 */
...
(function($) {
	...
	$.fn.extend({
		/**
		 * If used on document, returns the document's height (innerHeight) If
		 * used on window, returns the viewport's (window) height See core docs
		 * on height() to see what happens when used on an element.
		 * 
		...
		/**
		 * If used on document, returns the document's width (innerWidth) If
		 * used on window, returns the viewport's (window) width See core docs
		 * on height() to see what happens when used on an element.
		 * 
		...
		/**
		 * Returns the inner height value (without border) for the first matched
		 * element. If used on document, returns the document's height
		 * (innerHeight) If used on window, returns the viewport's (window)
		 * height
		 * 
		...
		innerHeight : function() {
			return this[0] == window || this[0] == document
					...
					: this.is(':visible') ? this[0].offsetHeight
							...
							- num(this, 'borderBottomWidth') : this.height()
							+ num(this, 'paddingTop')
		 ...
		 * element. If used on document, returns the document's Width
		...
		innerWidth : function() {
			return this[0] == window || this[0] == document
					...
					: this.is(':visible') ? this[0].offsetWidth
							...
							- num(this, 'borderRightWidth') : this.width()
		 ...
		 * matched element. Cannot be used on document or window.
		 * 
		...
		outerWidth : function() {
			return this[0] == window || this[0] == document
					? this.width()
					: this.is(':visible') ? this[0].offsetWidth : this.width()
							+ num(this, 'borderLeftWidth')
		 ...
		 */
		/**
		 * Sets the scrollLeft property and continues the chain. Works on
		...
		/**
		 * Returns the top and left positioned offset in pixels. The positioned
		 * offset is the offset between a positioned parent and the element
		 * itself.
		 * 
		 ...
		 * @name position
		 * @param Map
		 *            options Optional settings to configure the way the offset
		 *            is calculated.
		 * @option Boolean margin Should the margin of the element be included
		 *         in the calculations? False by default.
		 * @option Boolean border Should the border of the element be included
		 *         in the calculations? False by default.
		 * @option Boolean padding Should the padding of the element be included
		 *         in the calculations? False by default.
		 * @param Object
		 *            returnObject An object to store the return value in, so as
		 *            not to break the chain. If passed in the chain will not be
		 ...
		 * borders and padding.
		 * 
		 * Known issues: - Issue: A div positioned relative or static without
		 * any content before it and its parent will report an offsetTop of 0 in
		 * Safari Workaround: Place content before the relative div ... and set
		 * height and width to 0 and overflow to hidden
		 * 
		 ...
		 *            is calculated.
		 * @option Boolean margin Should the margin of the element be included
		 *         in the calculations? True by default.
		 * @option Boolean border Should the border of the element be included
		 *         in the calculations? False by default.
		 * @option Boolean padding Should the padding of the element be included
		 *         in the calculations? False by default.
		 * @option Boolean scroll Should the scroll offsets of the parent
		 *         elements be included in the calculations? True by default.
		 *         When true it adds the totla scroll offets of all parents to
		 *         the total offset and also adds two properties to the returned
		 *         object, scrollTop and scrollLeft.
		...
		offset : function(options, returnObject) {
			...
			if (elem.tagName.toLowerCase() == 'body') {
				// Safari is the only one to get offsetLeft and offsetTop
				// properties of the body "correct"
				// Except they all mess up when the body is positioned absolute
				// or relative
				x = elem.offsetLeft;
				y = elem.offsetTop;
				...
				do {
					...
					if (mo || ie) {
						...
						// Mozilla does not include the border on body if an
						// element isn't positioned absolute and is without an
						// absolute parent
						if (mo && parPos == 'absolute')
							absparent = true;
						// IE does not include the border on the body if an
						// element is position static and without an absolute or
						// relative parent
					...
					if (parent.tagName.toLowerCase() == 'body'
						...
						// Safari and IE Standards Mode doesn't add the body
						// margin for elments positioned with static or relative
						if ((sf || (ie && $.boxModel)) && elemPos != 'absolute'
								&& elemPos != 'fixed') {
						...
						// absolute parent
						// IE does not include the border on the body if an
						// element is positioned static and without an absolute
						// or relative parent
		...
		/**
		 * Returns the location of the element in pixels from the top left
		 * corner of the viewport. This method is much faster than offset but
		 * not as accurate. This method can be invoked by setting the lite
		 * option to true in the offset method.
		 * 
		 * @name offsetLite
		 * @param Map
		 *            options Optional settings to configure the way the offset
